...
auto MakeUSBIPDevice(
  uint32_t busId,
  uint32_t deviceId,
  const FREDEMMOTT_USB_VIRTPP_Device_DeviceConfig& config) {
  USBIP::Device ret{
    .mBusNum = busId,
    .mDevNum = deviceId,
...
extern "C" FREDEMMOTT_USB_VIRTPP_InstanceHandle FREDEMMOTT_USB_VIRTPP_Instance_Create(
  const FREDEMMOTT_USB_VIRTPP_Instance_InitData* initData) {
  auto ret = std::make_unique<FREDEMMOTT_USB_VIRTPP_Instance>(initData);
...
FREDEMMOTT_USB_VIRTPP_Instance::~FREDEMMOTT_USB_VIRTPP_Instance() {
  if (mNeedWSACleanup)
    WSACleanup();
}
...
uint16_t FREDEMMOTT_USB_VIRTPP_Instance_GetPortNumber(
  const FREDEMMOTT_USB_VIRTPP_InstanceHandle instance) {
  return instance->GetPortNumber();
}

uint16_t FREDEMMOTT_USB_VIRTPP_Instance::GetPortNumber() const {
  if (!mListeningSocket) {
...
void FREDEMMOTT_USB_VIRTPP_Instance_Run(
  const FREDEMMOTT_USB_VIRTPP_InstanceHandle instance) {
  instance->Run();
}

void FREDEMMOTT_USB_VIRTPP_Instance::Run() {
...
void FREDEMMOTT_USB_VIRTPP_Instance::HandleClient(const SOCKET clientSocket) {
...
std::expected<void, HRESULT> FREDEMMOTT_USB_VIRTPP_Instance::OnDevListOp() {
...
std::expected<void, HRESULT> FREDEMMOTT_USB_VIRTPP_Instance::OnImportOp(
...
std::expected<void, HRESULT> FREDEMMOTT_USB_VIRTPP_Instance::OnInputRequest(FREDEMMOTT_USB_VIRTPP_Device& device, const FredEmmott::USBIP::USBIP_CMD_SUBMIT& request, const FREDEMMOTT_USB_VIRTPP_Request apiRequest)
{
  ...
  if (FREDEMMOTT_USB_VIRTPP_SUCCEEDED(ret)) [[likely]] {
    return {};
  }
  return std::unexpected{std::bit_cast<HRESULT>(ret)};
}

std::expected<void, HRESULT> FREDEMMOTT_USB_VIRTPP_Instance::OnSubmitRequest(
  ...
  const FREDEMMOTT_USB_VIRTPP_Request apiRequest{
    .mDevice = &device,
  ...
  if (request.mSetup.mRequestType == 0) {
    ...
    switch (request.mSetup.mRequest) {
      case SetConfiguration: // no-op, we only support 1 configuration
        if (const auto ret = FREDEMMOTT_USB_VIRTPP_Request_SendReply(&apiRequest, 0, nullptr, 0); !FREDEMMOTT_USB_VIRTPP_SUCCEEDED(ret)) [[unlikely]] {
...
std::expected<void, HRESULT> FREDEMMOTT_USB_VIRTPP_Instance::OnUnlinkRequest(
...
void FREDEMMOTT_USB_VIRTPP_Instance_RequestStop(
  const FREDEMMOTT_USB_VIRTPP_InstanceHandle handle) {
...
FREDEMMOTT_USB_VIRTPP_InstanceHandle FREDEMMOTT_USB_VIRTPP_Device_GetInstance(
  const FREDEMMOTT_USB_VIRTPP_DeviceHandle handle) {
...
void FREDEMMOTT_USB_VIRTPP_RequestStopInstance(
  FREDEMMOTT_USB_VIRTPP_InstanceHandle instance) {
  instance->mStopSource.request_stop();
}

FREDEMMOTT_USB_VIRTPP_Result FREDEMMOTT_USB_VIRTPP_Request_SendReply(
  const FREDEMMOTT_USB_VIRTPP_Request* const request,
  ...
  [[unlikely]] {
    return std::bit_cast<FREDEMMOTT_USB_VIRTPP_Result>(ret.error());
  }
  if (actualLength == 0) {
    return FREDEMMOTT_USB_VIRTPP_SUCCESS;
  }
...
FREDEMMOTT_USB_VIRTPP_Device::FREDEMMOTT_USB_VIRTPP_Device(
  FREDEMMOTT_USB_VIRTPP_InstanceHandle instance,
  const FREDEMMOTT_USB_VIRTPP_Device_InitData* initData)
  : mInstance(instance) {
  if (!instance) {
    return;
  }
...
FREDEMMOTT_USB_VIRTPP_DeviceHandle FREDEMMOTT_USB_VIRTPP_Device_Create(
  const FREDEMMOTT_USB_VIRTPP_InstanceHandle instance,
  const FREDEMMOTT_USB_VIRTPP_Device_InitData* initData) {
  auto ret = std::make_unique<FREDEMMOTT_USB_VIRTPP_Device>(instance, initData);
  if (!ret->mInstance) {
    return nullptr;
  }
  return ret.release();
...
void FREDEMMOTT_USB_VIRTPP_Device_Destroy(
  const FREDEMMOTT_USB_VIRTPP_DeviceHandle handle) {
  delete handle;
}