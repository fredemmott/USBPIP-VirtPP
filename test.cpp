...
bool handle_devlist_request(SOCKET sock) {
  ...
  if (!send_all(sock, header))
    return false;
  if (!send_all(sock, MouseDevice))
    return false;
  if (!send_all(sock, MouseInterface))
    return false;
...
bool handle_import_request(SOCKET sock, const char* busid) {
  std::println(
    "-> Received OP_REQ_IMPORT for {}. Sending OP_REP_IMPORT.", busid);

  if (!send_all(sock, USBIP::OP_REP_IMPORT {.mDevice = MouseDevice}))
    return false;
...
bool handle_submit_request(
  SOCKET sock,
  const USBIP::USBIP_CMD_SUBMIT& command) {
  ...
  if (endpoint == 0) {
    ...
    std::println(
      "-> CMD_SUBMIT (Control EP0, Seq: {}, Type: 0x{:x}, Req: 0x{:x}, Len: "
      "{})",
    ...
    if (s.mRequestType == DeviceToHost && s.mRequest == 0x06) {
      ...
      std::println(
        "Getting type {} index {}", (int)descriptorType, (int)descriptorIndex);
    ...
    if (responseData) {
      ...
      USBIP::USBIP_RET_SUBMIT response {
        .mActualLength = to_network(actualLength),
        // 'use URB actual_length', intentionally not network byte order
      ...
      if (!send_all(sock, response))
        return false;
      if (!send_all(sock, responseData, actualLength))
        return false;
      return true;
    }
    ...
    std::println(
      stderr,
      "   - Error: Unhandled Control Request (0x{:x}/0x{:x}).",
  ...
  if (endpoint == 1 && command.mHeader.mDirection == USBIP::Direction::In) {
    ...
    if (!send_all(sock, response))
      return false;
    if (!send_all(sock, mouse_report, final_len))
      return false;
    return true;
  }
  ...
  std::println(
    stderr,
    "-> Error: Unhandled URB CMD (EP: {}, Dir: {}).",
    endpoint,
  ...
  while (true) {
    ...
    switch (request.mCommandCode) {
      case USBIP::CommandCode::OP_REQ_DEVLIST:
        if (!recv_remainder(client_socket, &request.mOP_REQ_DEVLIST))
          break;
        if (!handle_devlist_request(client_socket))
          break;
        continue;
      case USBIP::CommandCode::OP_REQ_IMPORT:
        if (!recv_remainder(client_socket, &request.mOP_REQ_IMPORT))
          break;
        if (!handle_import_request(
              client_socket, request.mOP_REQ_IMPORT.mBusID))
          break;
        continue;
      case USBIP::CommandCode::USBIP_CMD_SUBMIT:
        if (!recv_remainder(client_socket, &request.mUSBIP_CMD_SUBMIT))
          break;
        if (!handle_submit_request(client_socket, request.mUSBIP_CMD_SUBMIT))
          break;
        continue;
      case USBIP::CommandCode::USBIP_CMD_UNLINK: {
        if (!recv_remainder(client_socket, &request.mUSBIP_CMD_UNLINK))
          break;
    ...
    // We 'continue' from the successful cases, so if we reach here, we've
    // failed.
    break;
  }
...
int main() {
  ...
  std::println(
    "USB/IP Server listening on port {}...", ntohs(server_addr.sin_port));
